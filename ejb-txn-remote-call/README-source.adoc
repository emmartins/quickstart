...
:postgresVersion: :9.4
:postgresExtraArgs: -c max-prepared-transactions=110 -c log-statement=all
include::../shared-doc/run-a-local-postgresql-database.adoc[leveloffset=+2]

The {productName} servers need to be configured to be able to connect to the database.
First of all, a JDBC driver needs to be installed as https://docs.wildfly.org/{productVersion}/Developer_Guide.html#Class_Loading_in_WildFly[jboss module].
...
. Ensure the PostgreSQL database is running, as described in <<run_a_local_postgresql_database>>.
